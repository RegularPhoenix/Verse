...
function M.check_for_updates(args)
	...
	elseif remotec[1] == basec[1] then
		if not silent then
			vim.notify(
				"Unpushed commits detected. Push them or perform a reset in " .. vim.fn.stdpath("config"),
				vim.log.levels.INFO
			)
		end
		if not force then
		...
		if not silent then
			vim.notify("Paths diverged. Manual conflict resolution may be required.", vim.log.levels.WARN)
		end
		if not force then
			return 0
		end
...
function M.update_verse(args)
	...
	if M.check_for_updates({ silent = silent, force = force }) == 0 then
		return
	end

	local git_result = execute_git({ args = { "merge", "--ff-only", "--progress" } })
	if git_result ~= 0 then
		vim.notify(
			"Update failed! Please pull the changes manually in " .. vim.fn.stdpath("config"),
